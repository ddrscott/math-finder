...
var Cell = function Cell(row, col, max, seed) {
  this.row = row;
  this.col = col;
  this.max = max;
...
var App = {
  ...
  handleSeedChange: function handleSeedChange(e) {
   ...
   */
  markSums: function markSums(puzzle, origin) {
    var _this2 = this;
    ...
    dirs.forEach(function (dir) {
      origin.matches = _this2.sumDirection(puzzle, origin, dir);
      if (origin.matches) {
        var matchCls = 'match' + origin.matches.length;
        origin.matches.forEach(function (m) {
          return m.elm.addClass(matchCls);
        });
   ...
   */
  sumDirection: function sumDirection(puzzle, cell, dir) {
    var cells = [],
        sum = 0;
    do {
  ...
  generate: function generate(numRows, numCols) {
    var rows = [];
